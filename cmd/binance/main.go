...
func main() {
	...
	logger.Info("Starting Binance ingestor",
	...
	if !binanceCfg.Enabled {
		logger.Info("Binance ingestor is disabled, exiting")
		return
	}
	...
	healthServer.RegisterChecker("binance", func(ctx context.Context) health.ComponentHealth {
		h := binanceProvider.Health()
		status := health.StatusHealthy
		if !h.Healthy {
	...
	logger.Info("Binance ingestor started successfully",
		zap.Strings("pairs", binanceCfg.Pairs),
		zap.Float64("large_trade_threshold", binanceCfg.LargeTradeThresholdUSD),
	...
	logger.Info("Binance ingestor stopped")
}
